...
begin
  ...
  close;
  sql.Clear;
  Open('select produto.id, produto.descricao, produto.valor , produto.qtd, produto.categoria_id  '   +
', categoria.descricao from produto inner join categoria  '     +
'on produto.categoria_id = categoria.id'
   );
  FitGrid(DBGrid1);
  //sql.Add('where id = '+ T_id.Text);
  T_id.Text := ''; new_valor.Text := ''; new_qtd.Text := '';
...
'on produto.categoria_id = categoria.id'
  ...
  T_descricao.Text := ''; T_valor.Text := '' ; T_qtd.Text:= ''; T_cat.Text:= '';
end;
